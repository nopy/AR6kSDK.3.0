...
#ifdef ANDROID
tHCIUTILS_STATUS  (* pfn_HCIUTILS_RegisterHCINotification) ( tHCIUTILS_NOTIFICATION_TYPE t_type, int  nOpCode, tHCIUTILS_EVENT_CALLBACK  eventNotificationCallback ,void * p_appdata ) = NULL;
void ( *pfn_HCIUTILS_SendCmd) (tHCIUTILS_HCI_CMD   tCmd, void *  p_hci_cmd_params) = NULL;

void (*pfn_HCIUTILS_UnRegisterHCINotification)(tHCIUTILS_NOTIFICATION_TYPE t_type,int nOpCode) = NULL;
#endif /* ANDROID */
...
#ifdef ANDROID
void eventNotificationCallback ( tHCIUTILS_NOTIFICATION * pEvent)
{
...
#define LMP_FEATURES_LENGTH                   8
        ...
        if(pEvent->nOpCode ==  HCI_EVT_REMOTE_DEV_VERSION) {
	        A_UCHAR * eventPtr = (A_UINT32 *)pEvent->p_notification_data_buf;
            A_UINT32 len = pEvent->n_data_length;
...
static void
AbfRegisterToHciLib( ABF_BT_INFO * pAbfBtInfo)
{
    void *                                      handle = 0;
    ...
    handle = dlopen("hciutils.so", RTLD_NOW);
    if( handle == 0){
        A_DEBUG( "%s : Error loading library libandroid_runtime.so %s\n", __FUNCTION__, dlerror());
    }
    else
    {
        A_DEBUG( "Load hciutils.so successfully\n");
        pfn_HCIUTILS_RegisterHCINotification = dlsym(handle, "HCIUTILS_RegisterHCINotification");
        pfn_HCIUTILS_SendCmd = dlsym(handle, "HCIUTILS_SendCmd");
        pfn_HCIUTILS_UnRegisterHCINotification = dlsym(handle, "HCIUTILS_UnRegisterHCINotification");

        if ( (NULL == pfn_HCIUTILS_RegisterHCINotification) || (NULL == pfn_HCIUTILS_SendCmd) || 
              (NULL == pfn_HCIUTILS_UnRegisterHCINotification) )
        {
		    A_DEBUG("ERROR GETTING SYMBOLS \n");
        }else {
            for (i=0; i <= 2;i++) {      
...
Abf_BtStackNotificationDeInit(ATH_BT_FILTER_INSTANCE *pInstance)
{
    ...
    pfn_HCIUTILS_UnRegisterHCINotification(HCIUTILS_COMMAND|HCIUTILS_EVENT, 7);
    pfn_HCIUTILS_UnRegisterHCINotification(HCIUTILS_COMMAND|HCIUTILS_EVENT, 25);
#endif /* ANDROID */
...
BtEventThread(void *arg)
{
    ...
    if(pInfo->Flags & ABF_USE_ONLY_DBUS_FILTERING) {
    	...
    	AbfRegisterToHciLib(pAbfBtInfo);
    }
#endif /* ANDROID */

    /* Query the Sleep state of the WLAN device */
...
static A_STATUS SetupHciEventFilter(ABF_BT_INFO *pAbfBtInfo)
{
    ...
    do {
        ...
        if (1) {
#else
        if (!USE_DBUS_FOR_HEADSET_PROFILE(pAbfBtInfo)) {
#endif
...
#ifdef ANDROID
static A_STATUS IssueAFHViaHciLib (ABF_BT_INFO  * pAbfBtInfo,
...
#ifdef ANDROID
    if(pInfo->Flags & ABF_USE_ONLY_DBUS_FILTERING) {
        IssueAFHViaHciLib(pAbfBtInfo, CurrentWLANChannel);
    }
#endif /* ANDROID */
}
